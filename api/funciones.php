...
function editarUsuario($usuario){
	$bd = conectarBaseDatos();
	$sentencia = $bd->prepare("UPDATE usuarios SET rol = ?, correo = ?, nombre = ?, telefono = ? WHERE id = ?");
	return $sentencia->execute([$usuario->rol, $usuario->correo, $usuario->nombre, $usuario->telefono, $usuario->id]);	
}

function obtenerUsuarioPorId($idUsuario){
	$bd = conectarBaseDatos();
	$sentencia = $bd->prepare("SELECT id, rol, correo, nombre, telefono FROM usuarios WHERE id = ?");
	$sentencia->execute([$idUsuario]);
...
function obtenerRolUsuario($idUsuario) {
	$bd = conectarBaseDatos();
	$sentencia = $bd->prepare("SELECT rol FROM usuarios WHERE id = ?");
	$sentencia->execute([$idUsuario]);
	return $sentencia->fetchObject();
}
...
function obtenerUsuarios(){
	$bd = conectarBaseDatos();
	$sentencia = $bd->query("SELECT id, rol, correo, nombre, telefono FROM usuarios");
	return $sentencia->fetchAll();
}

function registrarUsuario($usuario){
	$bd = conectarBaseDatos();
	$sentencia = $bd->prepare("INSERT INTO usuarios (rol, correo, nombre, telefono, password) VALUES(?,?,?,?,?)");
	return $sentencia->execute([$usuario->rol, $usuario->correo, $usuario->nombre, $usuario->telefono, $usuario->password]);	
}
