  ...
  <section>
      ...
      <div class="level-left">
        <div class="level-item">
            <p class="title is-1 has-text-weight-bold">
                <b-icon icon="cash-register" size="is-large" type="personalizado"> </b-icon>
                Reporte de ventas
            </p>
    ...
    <div class="box">
      <p class="title is-4 has-text-weight-bold has-text-grey">
        <b-icon icon="account" size="is-medium" type="personalizado"> </b-icon>
      ...
      <b-field grouped group-multiline>
        ...
        >
          <b-taglist attached>
            ...
            <b-tag size="is-large" type="is-info">{{ usuario.total }}Bs</b-tag>
          </b-taglist>
        </div>
      </b-field>
    </div>
    ...
    >
      ...
      <b-table-column field="cambio" label="Cambio" sortable v-slot="props">
        {{ props.row.pagado - props.row.total }}Bs
      </b-table-column>

      <b-table-column field="total" label="Total" sortable v-slot="props">
        <strong>{{ props.row.total }}Bs</strong>
      </b-table-column>
      ...
      <b-table-column field="insumos" label="Insumos" v-slot="props">
        <b-table :data="props.row.insumos" :narrowed="true" class="is-size-7">
          ...
          <b-table-column field="cantidad" label="Cantidad" v-slot="props">
            {{ props.row.cantidad }} X {{ props.row.precio }}Bs
          </b-table-column>
          <b-table-column field="subtotal" label="Subtotal" v-slot="props">
            {{ props.row.cantidad * props.row.precio }}Bs
          </b-table-column>
        </b-table>
      </b-table-column>
...
import ExpostXlsFile from "export-from-json";
...
export default {
  ...
  methods: {
      ...
      worksheet.columns = [
        ...
        { header: 'Mesa', key: 'idMesa', width: 15 },
        { header: 'Fecha', key: 'fecha', width: 15 },
        { header: 'Atendió', key: 'atendio', width: 20 },
        { header: 'Cliente', key: 'cliente', width: 20 },
        { header: 'Pago', key: 'pagado', width: 15 },
      ...
      this.ventas.forEach(venta => {
        worksheet.addRow({
          ...
          pagado: venta.pagado,
          cambio: venta.pagado - venta.total,
          total: venta.total,
        });
      });
    ...
    DescargarPdf(){
        ...
        jsPDF: {unit:'in', format:'letter', orientation:'landscape'}, // Orientación de la página (portrait o landscape)
         };
